...
- [x] **Higher-level authoring layer M2-M4**: All four milestones shipped. M1 (`Segment`) 2026-04-24; M2 (`Effect`/`TryEffect`/`EffectPolicy`/`DryRun`/`RetryPolicy → RetryStrategy`) 2026-04-25; M3 (`RunSummary`/`WithFinalizer`/`Run` returns `(RunSummary, error)`) 2026-04-25; M4 (`DevStore`/`FromCheckpoint`/`WithDevStore`) 2026-04-25.
