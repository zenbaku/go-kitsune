...
- [x] **`ExpandMap[T]`** — recursive BFS expansion. `fn(item)` returns a new `*Pipeline[T]`; all emitted items are fed back through `fn` again until the inner pipeline is empty. The defining primitive for tree traversal, recursive API pagination, and graph walks. Emits items at depth N before depth N+1. fn may return nil for leaf nodes. `ExpandMapFunc` provided as a context-free convenience wrapper.
