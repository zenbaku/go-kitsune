...
- [x] **Effect outcomes through `Hook.OnItem`** *(shipped 2026-04-25, post-M3 follow-up)*: `Effect` now calls `hook.OnItem` per emitted outcome with the full per-item retry duration and the terminal error, plus `OnStageStart` / `OnStageDone`. `MetricsHook.Stages["my-effect"]` now populates with `Processed` (successes), `Errors` (terminal failures), and the latency histogram. `DryRun` skips hook calls entirely. `RunSummary.Outcome` derivation is unchanged (still uses `runCtx.effectStats`); the hook integration is purely additive.

- [x] **Inspector visualization for Segment and Effect** *(shipped 2026-04-25)*: Inspector dashboard updated for the M1-M2 graph metadata. Segment hulls (tinted dashed-border SVG groups with the segment name labelled above) group stages sharing a `segment_name`. Effect nodes carry a corner badge (`REQ` red for required, `BE` yellow for best-effort). Sidebar Configuration section expanded from 3 to 8 fields: Segment, Effect kind, Concurrency, Buffer, Overflow, BatchSize, Timeout, Retry policy / Supervision indicators.

- [x] **Inspector `RunSummary` surfacing** *(shipped 2026-04-25, M5)*: New optional `RunSummaryHook` interface (`OnRunComplete(ctx, RunSummary)`) defined in `runsummary.go`; `Runner.Run` fires it after finalizers complete. Inspector implements it, broadcasts a `summary` SSE event, includes the snapshot in `/state`, and replays to new SSE clients. UI renders a "Last Run" card (hidden until first run) with outcome badge, duration, completed-at, optional fatal error, collapsible finalizer-errors list. `InspectorStore` extended with `SaveSummary` / `LoadSummary` for refresh-survival; `SummarySnapshot` is the public type. Plan at `docs/superpowers/plans/2026-04-25-inspector-run-summary.md`.
...
- [ ] **Inspector visibility for replayed Segments**: When `WithDevStore` is attached and a snapshot exists, `tryReplaySegment` spawns an out-of-band goroutine that emits the cached items directly to the segment's output channel. The replay goroutine never goes through `OnStageStart` / `OnItem` / `OnStageDone`, so the inspector dashboard shows replayed segments as "pending" forever during a replayed run. Two reasonable shapes: (a) have `tryReplaySegment` / `captureSegment` fire a minimal set of hook events (OnStageStart, an OnItem per cached item with zero duration and nil error, OnStageDone with totals), or (b) add a new optional hook interface like `SegmentReplayHook.OnSegmentReplayed(name, count)` that the inspector renders as a distinct "replayed" stage status. Option (a) is invisible to existing hooks; option (b) is more honest about what happened. Decide which when picking this up.

- [ ] **Per-stage `EffectStats` in `RunSummary`**: `runCtx.effectStats` (per-Effect-stage atomic success/failure counters) is internal. Per-effect counts are now visible via `summary.Metrics.Stages["my-effect"]` after the M3 follow-up shipped, but the required/best-effort split is not preserved in the metrics snapshot (the latter is just `Processed`/`Errors`). For dashboards that want to render "required failures" and "best-effort failures" separately without re-deriving from stage names, add a structured `summary.EffectStats map[string]EffectStats` field where `EffectStats{Required bool; Success, Failure int64}`. Source-breaking addition to the `RunSummary` shape; small cost given pre-1.0.

- [ ] **`ReplayThrough(segmentName)` RunOption for `DevStore`**: Original M4 spec. Currently a user iterating on segment X manually deletes `X.json` and runs with `WithDevStore`; everything else replays from snapshot. A formal `ReplayThrough(name)` would declaratively express "replay everything required to produce X's input, then run X live and downstream live." Definition needs care for branched DAGs (segments on parallel forks): "everything upstream of X" is not totally ordered. Pick this up only when a concrete user case demands more than the manual-deletion idiom.

- [ ] **Idempotency-key-driven deduplication for `Effect`**: `EffectPolicy.Idempotent` and `EffectPolicy.IdempotencyKey func(any) string` are stored on the policy and propagated to stage metadata, but v1 does not de-duplicate retries against any backing store. Wire to a configurable `IdempotencyStore` (likely reusing the existing `Store` interface) so a retry against the same key after a successful first attempt becomes a no-op. Useful when the underlying effect target lacks server-side dedupe.

- [ ] **Tag a `v0.2` release**: M1-M5 substantially expand the public surface (`Segment`, `Composable`, `Effect`, `TryEffect`, `EffectPolicy`, `EffectOutcome`, `EffectOption`, `Required`, `BestEffort`, `AttemptTimeout`, `WithIdempotencyKey`, `EffectStageOption`, `DryRun`, `RunSummary`, `RunOutcome`, `WithFinalizer`, `RunSummaryHook`, `RunHandle.Summary`, `DevStore`, `FileDevStore`, `NewFileDevStore`, `WithDevStore`, `FromCheckpoint`, `ErrSnapshotMissing`, `inspector.SummarySnapshot`) plus one source-breaking signature change (`Run` returns `(RunSummary, error)`). Worth tagging a release with a CHANGELOG so M1-M5 work is referenceable. Include the `RetryPolicy` to `RetryStrategy` rename in the breaking-changes section.
